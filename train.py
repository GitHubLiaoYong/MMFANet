...
parser.add_argument('--fpn_dsn_loss', action='store_true', default=False,
                    help='multiple sfnet loss')
...
parser.add_argument('--split_loss', type=str, default='No',
                    help='use different lr for different component')
...
parser.add_argument('--poly_exp', type=float, default=1.0,
                    help='polynomial LR exponent')#这个可以改成0.9试试
parser.add_argument('--bs_mult', type=int, default=2,
                    help='Batch size for training per gpu')#每个GPU放两张图片
...
parser.add_argument('--mode', type=str, default='train',
                    help='whether use train and val data to train')
...
if args.apex:
    ...
    torch.distributed.init_process_group(backend='nccl',
                                         init_method='env://')
    # torch.distributed.init_process_group(backend='nccl',
    #                                      init_method='tcp://locaalhost:10000',
    #                                      rank=1,world_size=1)
...
def get_net(optimizer=None,criterion=None):
    ...
    net, _ = restore_snapshot(net,optimizer=optimizer,snapshot=args.snapshot,
...
def main():
    ...
    net = network.get_net(args, criterion)#损失函数在模型中
    #net = get_net(optimizer=optimizer,criterion=criterion)
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        if args.apex:#如果用了apex要调整采样器的epoch
