...
parser.add_argument('--split', type=str, default='val')#可以设置成test，当提交到服务器时
...
def sliding_window_cropping(data, scale=1.0):
    ...
    if scale < 1.0:
        scale = 1.0
    tile_size = (int(args.crop_size * scale), int(args.crop_size * scale))#args.crop_size=832

    overlap = args.sliding_overlap# overlap 重叠

    for img_ctr in range(len(data)):#对于每一张图片

        h, w = data[img_ctr].shape[1:]#获取图片的高和宽
        mapping[img_ctr] = [w, h, []]
...
def inference_sliding(model, img, scales):
    ...
    t_list = []
    for scale in scales:#对于所有的尺度

        target_w, target_h = int(w * scale), int(h * scale)#得到当前尺度
        scaled_img = img.resize((target_w, target_h), Image.BILINEAR)#对图片进行缩放
        y = []
        image_list = []
        flip_list = []
        for flip in range(flip_range):
            if flip:
                scaled_img = scaled_img.transpose(Image.FLIP_LEFT_RIGHT)#进行缩放之后还要进行翻转

            img_transform = transforms.Compose(
                [transforms.ToTensor(), transforms.Normalize(*mean_std)])#对图片进行归一化处理
            image = img_transform(scaled_img)
            image_list.append(image)
            flip_list.append(flip)

        mapping, input_crops = sliding_window_cropping(image_list, scale=scale)#得到的图片列表作为输入，以及图片尺寸
        ...
        with torch.no_grad():
            ...
            if hi >= args.crop_size:
                ...
                for b_idx in range(bi):
                    ...
                    cur_output = model(cur_input)#计算输出的
...
def setup_loader():
    ...
    if args.dataset == 'cityscapes':
        ...
        eval_scales = None
        if args.inference_mode == 'pooling':#默认是sliding
...
def main():
    ...
    for iteration, data in enumerate(pbar):
        ...
        runner.inf(imgs, img_names, gt, inference, net, scales, pbar, base_img)#运算模型得到分割结果
