...
parser.add_argument('--arch', type=str, default='network.MMFANet2.MMFANetV2_')
...
parser.add_argument('--snapshot', type=str, default='/home/liaoyong/PyCharm/DABNet-master/checkpoint/cityscapes/MMFANetV2_bs16gpu2_train_2020-11-05-20-23-44/model_1000.pth')
...
assert_and_infer_cfg(args, train_mode=False)#把args传递进去之后就会进行参数的推断，这里设置了数据集相关信息以及BN类型等
...
def sliding_window_cropping(data, scale=1.0):
    ...
    overlap = args.sliding_overlap#0.33333

    for img_ctr in range(len(data)):

        h, w = data[img_ctr].shape[1:]
        mapping[img_ctr] = [w, h, []]
        stride = ceil(tile_size[0] * (1 - overlap))#stride=555

        tile_rows = int(
            ceil((w - tile_size[0]) / stride) + 1)#tile_rows=2
        tile_cols = int(ceil((h - tile_size[1]) / stride) + 1)#tile_cols=1
...
def reverse_mapping(i, ctr, input_img, mapping, que, flip, origw, origh):
    ...
    # Accumulate and average overerlapping areas累积和平均重叠面积
    ...
    for scale in scales:
        ...
        with torch.no_grad():
            ...
            if hi >= args.crop_size:
                ...
                input_crops = input_crops.cuda()#一下子把一组图像输入，但其实这只是一张图片里的一个尺度
...
def get_net():
    ...
    # net.load_state_dict(args.snapshot)
